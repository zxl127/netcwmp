...
int cwmp_event_global_init(cwmp_t * cwmp)
{
    ...
    {
    	cwmp_log_error("access event global file fail");
        return CWMP_ERROR;
    }
...
int cwmp_event_file_save(cwmp_t * cwmp)
{
    ...
    {
        cwmp_log_error("can't open file: %s.", cwmp->event_filename);
        return CWMP_ERROR;
    }
    ...
    cwmp_log_info("cwmp_event_file_save event_global.event_flag=0x%x", cwmp->event_global.event_flag);
...
int cwmp_event_init(cwmp_t *cwmp)
{
    ...
    if(!cwmp)
    {
        cwmp_log_error("param cwmp_event is NULL\n");
        return CWMP_ERROR;
    }
    ...
    cwmp_log_info("cwmp_event_init event_global.event_flag=0x%x", cwmp->event_global.event_flag);

    //upgrade firmware
    if(cwmp->event_global.event_flag & EVENT_REBOOT_TRANSFERCOMPLETE_FLAG)
    {
    	cwmp_log_info("cwmp_event_init EVENT_REBOOT_TRANSFERCOMPLETE_FLAG");
    	
    	cwmp_event_set_value(cwmp, INFORM_BOOT, 1, NULL, 0, 0, 0);
        cwmp_event_set_value(cwmp, INFORM_MDOWNLOAD, 1, NULL, 0, 0, 0);
        cwmp_event_set_value(cwmp, INFORM_TRANSFERCOMPLETE, 1, cwmp->event_global.event_key, cwmp->event_global.fault_code, cwmp->event_global.start, time(NULL));
    }
	else if(cwmp->event_global.event_flag & EVENT_REBOOT_ACS_FLAG){
		cwmp_event_set_value(cwmp, INFORM_BOOT, 1, NULL, 0, 0, 0);
    ...
    if(cwmp->event_global.event_flag){
...
int cwmp_event_set_value(cwmp_t *cwmp,  int event,   int value, const char * cmd_key, int fault_code, time_t start, time_t end)
{
    ...
    {
        cwmp_log_error( "set wrong event %d", event);
        return CWMP_ERROR;
    }
    int count = cwmp->el->count;
	
	cwmp_log_debug( "event=%d, value=%d", event, value);
		
    if(count  >= cwmp->el->size)
    {
     	cwmp_log_error( "event_count=%d, event_max_count=%d", count, cwmp->el->size);
		return CWMP_ERROR;
    }
...
int cwmp_event_clear_active(cwmp_t *cwmp)
{
	...
	switch(pec[i]->event)
	{
		...
		case INFORM_DIAGNOSTICSCOMPLETE:
			//cwmp->event_global.event_flag |= EVENT_REBOOT_TRANSFERCOMPLETE_FLAG;
            memset(cwmp->event_global.event_key, 0, COMMAND_KEY_LEN+1);
			strcpy(cwmp->event_global.event_key, pec[i]->command_key);
			cwmp->event_global.fault_code = pec[i]->fault_code;
